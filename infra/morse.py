...
def decode_morse(encoded: str) -> Tuple[str]:
    ...
    def decode_sentence(encoded_sentence: str) -> str:
        return " ".join(
            ...
            for encoded_word in encoded_sentence.split(WORD_DELIMITER)
        )

    return tuple(
        decode_sentence(encoded_sentence.strip())
        for encoded_sentence in encoded.split(SENTENCE_DELIMITER)
    )
